...
func TestCouponFlow_NonFungible(t *testing.T) {
	...
	voucherOwner := ownerSigner.PublicKey
	...
	assert.Equal(t, address, coupon.Address, "coupon address mismatch")
	assert.NotEmpty(t, coupon.TokenAddress, "coupon token address should not be empty")
	...
	assert.NotEmpty(t, balanceStates[0].TokenUUID, "balance token uuid should not be empty for NFT token")
	...
	assert.Equal(t, paused, updatedCoupon.Paused, "updated coupon paused mismatch")
	...
	issueOutput, err := c.IssueVoucher(coupon.Address, ownerSigner.PublicKey, issueAmount)
	if err != nil {
	...
	assert.Equal(t, ownerSigner.PublicKey, issuedVoucher.ToAddress, "issued voucher recipient address mismatch")
	...
	assert.Equal(t, ownerSigner.PublicKey, redeemedVoucher.UserAddress, "redeemed voucher sender address mismatch")
